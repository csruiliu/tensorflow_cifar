...
class AlexNet:
    ...
    def build(self, model_input):
        with tf.variable_scope('conv_1'):
            x = tf.keras.layers.Conv2D(filters=64, kernel_size=3, strides=2, padding='same')(model_input)
            x = tf.keras.activations.relu(x)
            x = tf.keras.layers.MaxPool2D(pool_size=2)(x)

        with tf.variable_scope('conv_2'):
            x = tf.keras.layers.Conv2D(filters=192, kernel_size=3, strides=1, padding='same')(model_input)
            x = tf.keras.activations.relu(x)
            x = tf.keras.layers.MaxPool2D(pool_size=2)(x)
